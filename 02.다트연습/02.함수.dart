void main(List<String> args) {
  ...
  // 다음추가 이순신 시리즈가 있다면 어느배우를 추천하는가?
  showTxt("다음 이순신 시리즈가 있다면 추천배우는 공유다!");
  showTxt('공유의 취미는 ${recommActor['공유']?['취미']}다. 사는곳은 ${recommActor['공유']?['사는곳']}다.자동차는 ${recommActor['공유']?['자동차']??'정보없음'}이다!');

  // 익명함수를 만들어서 특정함수에 값으로 전달한다!
  japanShip((){print('개.박.살...! 일본배 침몰!');});

  // 람다함수
  showTxt('"아직 신에게는 12척의 배가 남아있습니다!" 이 대사가 나오는 이순신의 전투는? ${leeFight[0]}해전 입니다.');
  ...
  showTxt('이순신의 부하중 이순신이 있었다. 그는 전투전에 너무 긴장되어서 구구단을 외웠다! ${shipNum+3}단!');
  } // main 
