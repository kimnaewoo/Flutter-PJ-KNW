...
void main(List<String> args) {
  ...
  print('내 강아지의 색깔은 ${d1.color}이고 나이는 ${d1.age}이다!');
  ...
  // 기존 d1과 연결성은 없다!
  Dog d2 = Dog();
  print('나의 사랑하는 강아지는 강아지 종류는 ${d2.name}이다!');

  // 값을 생성시에 초기화하는 Cat 사용하기 
  Cat c1 = Cat("코리안숏헤어", 4, "갈색얼룩");
  print('나의고양이 종은${c1.name} 이고 나이는 ${c1.age}살, 색깔은 ${c1.color}이다!');
  // c1.name = '이집트고양이'; -> final이면 재할당이 안된다!
  print('나의 고양이종을 다시 말하면 ${c1.name}이다!');

  // 상수 테스트 : const, final
  const aa = "aa";
  // const bb; -> 선언과 동시에 할당해야함!
  final bb; // final은 선언후 처음 할당되는 것을 상수화한다.
  bb = "bb";
  // bb = "bbb"; -> 재할당은 불가
  print('${aa}-${bb}');
...
/****************************************** 
 [ 다트의 클래스 ]
 - 정의 : 
  목적을 가진 프로그램에서 사용하기 위해 구성요소에 해당하는 변수와 함수로 구성된 프로그램 단위체!
  특히, 클래스 내부의 변수를 속성(멤버)라고 한다.
  특히, 클래스 내부의 함수를 메서드라고 한다.
  
 - 형식 : 
  class 키워드를 사용하여 파스칼케이스로 명명한다.
  예) 
  ...
  [ 다트의 클래스 상속 ]
  - 부모클래스의 모든 속성과 메서드를 자식클래스에서 사용하고자 할때 
    기본 공유를 하도록 하는 클래스 장치 
  - 상속 설정방법 : 
  class 클래스명 extends 부모클래스명 {
      구현코드...
  }
  -> 만약 부모클래스 멤버필드가 초기화가 필요한 경우 상속받은 클래스 생성시, 
  반드시 부모클래스 생성자를 먼저 호출하기 때문에 부모 클래스의 값을 먼저 초기화 셋팅해야 한다.
...
class Cat extends Pet{ 
  ...
  // 상속받은 부모 멤버필드 초기화는 이니셜라이즈(initialize) 키워드 콜론(:)을 사용하여 
  // 그뒤에 super키워드(부모클래스)로 값을 초기화함!
